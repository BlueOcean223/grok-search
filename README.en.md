...
- [Responses mode](docs/responses-mode.md)
...
| `GROK_API_PROVIDER` | `apiProvider` | No | `search.js` | `xai`, `openrouter`, or `openai-compatible`. Inferred from URL when omitted. |
| `GROK_MODEL` | `model` | No | `search.js` | Defaults to `grok-4-fast`. |
| `GROK_SEARCH_MODE` | `searchMode` | No | `search.js` | `chat` or `responses`. Default: `chat`. |
| `GROK_RESPONSES_MAX_TURNS` | `responsesMaxTurns` | No | Responses | Responses agentic turn limit. Default: `1`. |
...
| `GROK_RESPONSES_FALLBACK_CHAT` | `responsesFallbackChat` | No | Responses | Explicitly fall back to Chat if Responses fails. Default: `false`. |
...
In Chat mode, OpenRouter models automatically get `:online` unless the model name already has that suffix. Responses mode uses the `openrouter:web_search` server tool and does not append `:online`.
...
`search.js` defaults to `chat` mode, calls `{GROK_API_URL}/chat/completions` with `stream:false`, injects local time context, and returns:
...
- `diagnostics.grok_endpoint`, `diagnostics.warnings`, `diagnostics.provider_attempts`, `diagnostics.options`, and `diagnostics.searched_at`

Optional `responses` mode calls `{GROK_API_URL}/responses` and enables the provider's server-side web search tool. It can return richer citation/searched-source structure, but may cost significantly more than Chat, so it is never the default. Responses mode exposes the actual path and cost through `diagnostics.options.search_mode`, `diagnostics.grok_endpoint`, `diagnostics.responses_*`, `diagnostics.cost_in_usd_ticks`, and `diagnostics.cost_usd`.
...
There is no formal `both` mode. To compare Chat and Responses, run the two commands separately and compare the resulting JSON yourself.
